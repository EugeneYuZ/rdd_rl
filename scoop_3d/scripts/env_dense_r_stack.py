...
class ScoopEnv:
    ...
    def __init__(self, port=19997):
        ...
        self.cube = None
        self.cube_pos_range = [[-0.35, 0.05], [0.6, 0.85]]
    ...
    def reset(self):
        ...
        x = np.random.uniform(self.cube_pos_range[0][0], self.cube_pos_range[0][1])
        y = np.random.uniform(self.cube_pos_range[1][0], self.cube_pos_range[1][1])

        utils.setObjectPosition(self.sim_client, self.cube, [x, y, 0.025])
    ...
    def getReward(self):
        ...
        return -np.linalg.norm(tip_position-cube_position)
    ...
    def step(self, a):
        ...
        if -0.42 < target_position[0] < 0 and 0.42 < target_position[1] < 0.95 and 0 < target_position[2] < 0.3:
        ...
        if not (self.cube_pos_range[0][0] - self.cube_size[0] < cube_position[0]
                < self.cube_pos_range[0][1] + self.cube_size[0] and
                self.cube_pos_range[1][0] - self.cube_size[1] < cube_position[1]
                < self.cube_pos_range[1][1] + self.cube_size[1] and
                0.025 - self.cube_size[2] < cube_position[2] < 0.025 + self.cube_size[2] and
                cube_orientation[0] < 0.1):
            # print 'Wrong cube position: ', cube_position
            return None, -10, True, None

        # cube is lifted
        if cube_position[1] > 0.83 and cube_position[2] > 0.03:
            return None, +10, True, None
